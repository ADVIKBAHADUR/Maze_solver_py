...
function resolve_position(position_config, fallback_x_ratio, fallback_y_ratio) {
    ...
    if (mode === "fixed" &&
        Number.isInteger(config.x) &&
        Number.isInteger(config.y)) {
        x = config.x;
        y = config.y;
        ...
        let x_ratio = typeof config.xRatio === "number" ? config.xRatio : fallback_x_ratio;
        let y_ratio = typeof config.yRatio === "number" ? config.yRatio : fallback_y_ratio;

        x = Math.floor(grid_size_x * x_ratio);
        y = Math.floor(grid_size_y * y_ratio);
    }

    x = clamp(x, 0, grid_size_x - 1);
    y = clamp(y, 0, grid_size_y - 1);

    let x_if_even_shift = Number.isInteger(config.xIfEvenShift) ? config.xIfEvenShift : 1;
    let y_if_even_shift = Number.isInteger(config.yIfEvenShift) ? config.yIfEvenShift : -1;

    if (x % 2 == 0)
        x = clamp(x + x_if_even_shift, 0, grid_size_x - 1);

    if (y % 2 == 0)
        y = clamp(y + y_if_even_shift, 0, grid_size_y - 1);

    return [x, y];
}

function set_grid_properties() {
    let grid_config = (window.MAZE_CONFIG && window.MAZE_CONFIG.grid) ? window.MAZE_CONFIG.grid : {};

    if (grid_config.mode === "fixed" &&
        Number.isInteger(grid_config.width) &&
        Number.isInteger(grid_config.height) &&
        grid_config.width > 1 &&
        grid_config.height > 1) {
        grid_size_x = grid_config.width;
        grid_size_y = grid_config.height;

        let available_width = window.innerWidth - menu_width;
        let max_cell_width = Math.floor(available_width / grid_size_x);
        let max_cell_height = Math.floor(window.innerHeight / grid_size_y);
        cell_size = Math.max(1, Math.min(max_cell_width, max_cell_height));
        return;
    }
...
function generate_grid() {
    ...
    let positions = (window.MAZE_CONFIG && window.MAZE_CONFIG.positions) ? window.MAZE_CONFIG.positions : {};
    start_pos = resolve_position(positions.start, 0.25, 0.5);
    target_pos = resolve_position(positions.end || positions.stop, 0.75, 0.5);

    if (start_pos[0] == target_pos[0] && start_pos[1] == target_pos[1]) {
        let shifted_x = clamp(target_pos[0] + 2, 0, grid_size_x - 1);

        if (shifted_x == start_pos[0])
            shifted_x = clamp(target_pos[0] - 2, 0, grid_size_x - 1);

        target_pos = [shifted_x, target_pos[1]];
    }
